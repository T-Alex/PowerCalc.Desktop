...
namespace _3DTools
{
    public class TrackballDecorator : Viewport3DDecorator
    {
        ...
        /// <summary>
        ///     A transform to move the camera or scene to the trackball'sum
        ...
        private Vector3D ProjectToTrackball(double width, double height, Point point)
        {
            ...
            double z = point.Y / (height / 2);

            x = x - 1;                           // Translate 0,0 to the center
            z = z - 1;                           // Flip so +Y is up instead of down

            double y2 = 1 - x * x - z * z;       // z^2 = 1 - x^2 - y^2
            double y = (y2 > 0) ? Math.Sqrt(y2) : 0;
        ...
        private void Zoom(int delta)
        {
            ...
            double scale = Math.Exp(yDelta / 2000);    // e^(yDelta/1000) is fairly arbitrary.
