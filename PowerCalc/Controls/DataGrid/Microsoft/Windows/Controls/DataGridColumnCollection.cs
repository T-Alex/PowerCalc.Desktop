...
namespace Microsoft.Windows.Controls
{
    ...
    internal class DataGridColumnCollection : ObservableCollection<DataGridColumn>
    {
        ...
        protected override void InsertItem(int index, DataGridColumn item)
        {
            if (item == null)
            {
                throw new ArgumentNullException("item", "Cannot add a null column to a DataGrid's column collection.");
            }

            if (item.DataGridOwner != null)
            {
                throw new ArgumentException(String.Format("DataGridColumn with Header '{0}' already exists in the Columns collection of a DataGrid. DataGrids cannot share columns and cannot contain duplicate column instances.", item.Header), "item");
            }
        ...
        protected override void SetItem(int index, DataGridColumn item)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("index", String.Format("The index for the DataGridColumn with Header '{0}' is out of range. Index must be greater than or equal to 0 and less than Columns.Count.", item.Header));
            }
        ...
        internal void InitializeDisplayIndexMap()
        {
            ...
            for (int columnIndex = 0; columnIndex < columnCount; columnIndex++)
            {
                ...
                if (currentColumnDisplayIndex >= 0)
                {
                    ...
                    {
                        throw new ArgumentException("DisplayIndex is already used for other column.  DisplayIndex should be unique per column.");
                    }
        ...
        internal void ValidateDisplayIndex(DataGridColumn column, int displayIndex, bool isAdding)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("displayIndex", displayIndex, String.Format("The DisplayIndex for the DataGridColumn with Header '{0}' is out of range.  DisplayIndex must be greater than or equal to 0 and less than Columns.Count.", column.Header));
            }
        }
